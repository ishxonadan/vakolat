...
const activeCount = computed(() => categories.value.filter((row) => row.isActive !== false).length)

const loadCatalog = async () => {
  loading.value = true
  try {
    categories.value = await apiService.get("/member-categories", { params: { includeInactive: true } })
  } catch (e) {
    toast.add({
      severity: "error",
      summary: "Xato",
      detail: e?.message || "Toifalar yuklanmadi",
      life: 3500,
    })
  } finally {
    loading.value = false
...
const addNew = async () => {
  ...
  try {
    await apiService.post("/member-categories", { code, name })
    newCode.value = ""
    newName.value = ""
...
const saveRow = async (row, { silent = false } = {}) => {
  try {
    await apiService.put(`/member-categories/${row._id}`, {
      code: row.code,
      name: row.name,
...
const deleteRow = async (row) => {
  if (!window.confirm(`"${row.name}" toifasini o‘chirmoqchimisiz?`)) return
  try {
    await apiService.delete(`/member-categories/${row._id}`)
  ...
  <div class="min-h-full bg-slate-50/90 dark:bg-surface-950 pb-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div class="space-y-2">
            ...
            <p class="text-surface-600 dark:text-surface-400 max-w-2xl text-sm sm:text-base leading-relaxed">
              A'zolarni ro‘yxatga olishdagi toifa ro‘yxati. Faol toifalar formadagi dropdownda chiqadi; nofaol
              qilinganlari tanlanmaydi.
            </p>
          </div>
          <div
      ...
      >
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:gap-4 max-w-3xl">
          <div class="w-full sm:w-36">
            <label for="member-cat-new-code" class="mb-1.5 block text-sm font-medium text-surface-700 dark:text-surface-300">
              Kod
            </label>
            <InputText
              id="member-cat-new-code"
              v-model="newCode"
              placeholder="0019"
              class="w-full"
              maxlength="4"
          ...
          <div class="flex-1 min-w-0">
            <label for="member-cat-new-name" class="mb-1.5 block text-sm font-medium text-surface-700 dark:text-surface-300">
              Nom
            </label>
            <InputText
              id="member-cat-new-name"
              v-model="newName"
              placeholder="Masalan: Tadqiqotchi"
              class="w-full"
              @keyup.enter="addNew"
            />
          </div>
      ...
      >
        ...
        >
          <h2 class="text-sm font-semibold text-surface-800 dark:text-surface-200">Barcha toifalar</h2>
          <Button
            type="button"
            icon="pi pi-refresh"
            severity="secondary"
        ...
        <div v-if="loading && categories.length === 0" class="flex flex-col items-center justify-center gap-3 py-16">
          <ProgressSpinner style="width: 2.5rem; height: 2.5rem" stroke-width="4" />
          <span class="text-sm text-surface-500">Yuklanmoqda…</span>
        </div>

        <div v-else-if="categories.length === 0" class="py-14 text-center">
          <p class="font-medium text-surface-700 dark:text-surface-300">Hozircha toifa yo‘q</p>
        </div>
        ...
        <ul v-else class="divide-y divide-surface-200 dark:divide-surface-700">
          ...
          >
            <div class="w-24 shrink-0">
              <InputText v-model="row.code" class="w-full font-mono" maxlength="4" />
            </div>
            <div class="flex-1 min-w-0">
