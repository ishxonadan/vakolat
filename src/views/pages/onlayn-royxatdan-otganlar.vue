...
const fetchMembers = async () => {
  try {
    ...
    console.error('Error fetching online registrants:', error)
    toast.add({
      severity: 'error',
      summary: 'Xatolik',
      detail: "Onlayn ro'yxatdan o'tganlarni yuklashda xatolik yuz berdi",
      life: 3000
    })
  } finally {
    loading.value = false
...
const saveMember = async (updatedMember) => {
  try {
    if (isEditMode.value) {
      await apiService.put(`/online-registrants/${updatedMember.USER_NO}`, updatedMember)
      toast.add({
        severity: 'success',
      ...
      await apiService.post('/online-registrants', updatedMember)
      toast.add({
        severity: 'success',
        summary: 'Muvaffaqiyatli',
        detail: "Yangi foydalanuvchi qo'shildi",
        life: 3000
      })
    }
    ...
    console.error('Error saving online registrant:', error)
    toast.add({
      severity: 'error',
      summary: 'Xatolik',
