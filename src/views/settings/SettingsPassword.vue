  ...
  <div>
    ...
    <div class="space-y-4 max-w-xl">
      ...
      <div>
        ...
        <p class="text-xs text-gray-500 mt-1">Minimal talab: kamida 6 ta belgi.</p>
      </div>

      <div>
        <label for="confirmPassword" class="block text-sm font-medium mb-1">Yangi parolni tasdiqlang</label>
        <InputText id="confirmPassword" v-model="confirmPassword" type="password" class="w-full" />
      </div>
    </div>

    <div class="mt-6 flex justify-end max-w-xl">
      <Button label="Parolni o'zgartirish" icon="pi pi-key" @click="changePassword" :loading="loading" />
    </div>
  </div>
</template>
